...
interface OrganizerWalletCardProps {
  onWalletConnected?: (address: string) => void;
  compact?: boolean;
}

export function OrganizerWalletCard({ onWalletConnected, compact = false }: OrganizerWalletCardProps) {
  const {
    isConnected,
    address,
    balance,
  ...
  useEffect(() => {
    if (isConnected && address && address !== savedWallet) {
      saveOrganizerWallet(address);
    }
  }, [isConnected, address]);

  const loadSavedWallet = async () => {
    try {
      const response = await apiClient.request('/api/organizer/profile');
      if (response?.walletAddress) {
        setSavedWallet(response.walletAddress);
      }
    } catch (err) {
  ...
  const saveOrganizerWallet = async (walletAddress: string) => {
    setSaving(true);
    try {
      await apiClient.request('/api/organizer/wallet', {
        method: 'PUT',
      ...
      toast({
        ...
        description: 'You will receive 95% of ticket sales directly to this wallet.',
      });
    } catch (err) {
      console.error('Failed to save wallet:', err);
      toast({
        title: 'Failed to save wallet',
        description: 'Could not update wallet. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setSaving(false);
  ...
  return (
    <Card className="border-purple-500/30 bg-gradient-to-br from-purple-500/10 to-cyan-500/10">
        ...
        {!isMetaMaskInstalled ? (
          <div className="p-4 bg-red-500/20 border border-red-500/30 rounded-lg">
            ...
            <p className="text-sm text-gray-400 mb-3">
              Install MetaMask to receive ticket sale payments directly.
            </p>
            <Button
          ...
          <div className="space-y-3">
            <div className="p-4 bg-white/5 rounded-lg text-center">
              ...
              <p className="text-xs text-gray-500">
                Connect your MetaMask to receive 95% of ticket sales directly
              </p>
            </div>
            <Button
              onClick={handleConnect}
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-lg">
              <div className="flex items-center gap-2 text-green-300 mb-1">
                ...
                <span className="text-sm font-semibold">Revenue Split</span>
              </div>
              <p className="text-xs text-gray-400">
                When buyers purchase tickets, 95% goes directly to your wallet, 5% platform fee.
              </p>
            </div>
